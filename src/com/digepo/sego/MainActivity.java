...
public class MainActivity extends Activity{
	...
	public void getLocalidad() throws IOException {
		...
		try {
			...
			/*else if(localidad.equals("Dalvik")) {
				...
				else { /* Indica que trae un vecindario y hay que mostrar una lista de coincidencias. */
					try {
    ...
    /**
     * Obtiene coincidencias de municipios en el servidor y y los muestra.
     * 
     * @see MyUtils, getDetallesMunicipio
     * @return void
     * @throws Exception 
     */
    public void muestra_coincidencias() throws Exception {
    	getDetallesMunicipio detMunicipio = new getDetallesMunicipio();
		...
		try {
			...
			if(result.equals("OK")){ //validation result OK
				String municipios = new String(mcrypt.decrypt(jObjDet.getString("municipios").toString()));
				String[] municipios_array= municipios.split(";");
				
				ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,R.layout.list_item,R.id.product_name,municipios_array);
				
				ListView lista = new ListView(this);
				lista.setAdapter(adapter);
				
				//lista.setOnClickListener(l)
			}
			//Log.e("JSON original",detalles);
		} catch (InterruptedException e) {		
			e.printStackTrace();
